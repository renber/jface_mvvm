 ...
 *                    266038
 *     René Bergelt - TreeNode does not dispose its backing observable collection if it is not owned by the TreeNode (avoids disposing ViewModel properties)
 *     				  Classes which should not be disposed need to implement IViewModelOwned
 ******************************************************************************/
 ...
 * @since 1.2
 */
public abstract class ObservableCollectionTreeContentProviderEx implements
	 ...
	 */
	protected ObservableCollectionTreeContentProviderEx(
	...
	protected final class TreeNode {
		...
		private void dispose() {
			...
			if (children != null && !children.isDisposed()) {
				...
				// Only dispose the chidlren collection if it is not owned by a ViewModel
				if (!(children instanceof IViewModelOwned))
						children.dispose();
